...
OUTPUT_DIR = os.path.join(
    DATA_DIR, "02_results"
)  # data/04_ambiguity_detection/02_results に出力
...
def generate_prompt(text):
    """
    LLM-JP用のプロンプト（Few-Shot Example付き）
    過剰反応を防ぐため、判断可能な例と曖昧な例を提示する。
    """
    instruction = f"""
あなたは日本語の道徳的判断データセットの品質管理者です。
与えられた「対象の文章」だけを読んで、「道徳的に許容できる」か「許容できない」かを判断するのに、**必要な情報が不足している（曖昧である）か**を判定してください。

判定基準:
- TRUE (情報不足): 状況や前提条件が不明瞭で、これだけでは判断できない、または人によって判断が大きく分かれる場合。
- FALSE (情報十分): 常識的に考えて、その行為が良いか悪いか判断できる場合。文脈が多少不足していても、一般的な状況を想定すれば判断できるならFALSEとしてください。

例1:
文章: 「隣人に新鮮なリンゴをおすそ分けした」
判定: FALSE
(解説: 一般的に良い行為と判断できるため)
...
文章: 「現実との区別がつかなくなる」
判定: TRUE
(解説: 病院で麻酔後なのか、違法薬物を摂取したためなのか、これだけでは判断が難しいため)
...
以下の対象の文章について、判定（TRUE または FALSE）のみを出力してください。
